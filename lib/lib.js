...
exports.post = (req, res) => {
  ...
  let searching = (user, next) => {
    ...
    Object.keys(history).forEach(function (key) {
      if (history[key].first_name === user.name && history[key].last_name === user.last_name && history[key].last_name2 === user.last_name2 || (user.name === history[key].first_name + ' ' + history[key].second_name)) {
  ...
  async.auto({
    ...
    getThisYear: (cb) => {
      ...
      let query = 'SELECT name,last_name,last_name2, number  FROM thisyear LIMIT 100';
      connection.query(query, (error, results) => {
        if (error) {
